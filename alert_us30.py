...
def get_events():
    # SOURCE 1: ForexFactory (via Faireconomy)
    url_ff = "https://nfs.faireconomy.media/ff_calendar_thisweek.json"
    # SOURCE 2: Proxy Investing (Alternative)
    # Note: En gratuit, on utilise souvent des agrégateurs de JSON
    
    events = []
    try:
        resp = requests.get(url_ff, timeout=15, headers={"User-Agent": "Mozilla/5.0"})
        data = resp.json()
